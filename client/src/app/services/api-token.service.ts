...
createToken(tokenName:string, tokenDescription:string):  Observable<any> {
  return this.http.post(this.createTokenUrl, {name: tokenName, description:tokenDescription}, this.httpOptions);/*.pipe(
    tap((newToken: ApiToken) => this.log(`created token w/ id=${newToken.id}`)),
    catchError(this.handleError<ApiToken>('createToken'))
  );*/
}
...
/** PUT: update the token on the server */
...
validateToken(id: string): Observable<ApiToken> {
  ...
  console.log("validateToken"+url+ " "+id);
  return this.http.get<ApiToken>(url).pipe(
